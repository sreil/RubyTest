module Dict
	...
	def Dict.hash_key(aDict, key) #Given a key this will create a number and then convert it to an index for the aDict's buckets.
	...
	def Dict.get_slot(aDict, key, default = nil) # Returns the index, key, and value of a slot found in a bucket.
		bucket_id = Dict.get_bucket(aDict, key)

		buckets.each_with_index do |kv, i|
			k, v = kv
			if key == k
				return i, k, v
			end
